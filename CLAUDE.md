...
- `data/liveness.py::live_session_procs()` is the ONE assembly point that joins the two: registry `SessionProc` rows with `proc_alive` injected via `pid_alive`. A `SessionProc.proc_alive` is only trustworthy after this injection (the registry parse leaves it `False`) — always come through this seam instead of re-inlining `replace(sp, proc_alive=pid_alive(...))`.
