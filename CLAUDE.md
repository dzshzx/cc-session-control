...
**Foreground tmux resume (`t` key):** the middle tier between `Enter` (bare-terminal resume, dies with the terminal) and `R` (background + remote control): resume the session inside its per-project tmux session (`tmux.session_name_for(cwd)` — one tmux session per project, windows per claude; plain `claude --resume`, deliberately **no `--remote-control`** — every RC process mints a new cloud env entry) and bring the user's terminal INTO that window, so a dropped SSH/phone connection no longer kills the session. A session already living in a tmux pane (found via `tmux.find_session_window` — pane-pid ∈ the session pid's `/proc` ancestor chain, any tmux session) is entered **in place** (no kill, no confirm, no R10 gate); otherwise it follows the same `would_take_over`/confirm/R10 path as `R`, then `cli.py` spawns the window (`do_tmux_resume`) and enters it. Entering = `enter_window`: outside tmux `exec tmux attach-session` (replaces csctl, like `do_resume`); inside tmux (`$TMUX` set) `switch-client` then csctl exits normally — both paths end csctl ("接回 = 离开 csctl").

**Relaunch into tmux (`R` key):** `relaunch_in_tmux` (in `actions/session_ops.py`) runs `claude --resume <sid> --remote-control <name>` in a tmux window (in the per-project session `tmux.session_name_for(cwd)`, created via `tmux.run_in_tmux`, which returns the exact "session:window_index" target via `-P` — kept separate from the `rc` server windows) so the session **outlives the terminal** and is controllable from phone / claude.ai/code. Unlike `do_resume` it does **not** replace the csctl process — it just spawns the window, reusing the same `should_kill` handoff (live non-current session → old pid killed first; current session refused). Gotcha learned from a spike: killing the tmux window does **not** reliably kill a `--remote-control` process (it survives orphaning), so stop a relaunched session via the `s` terminate-by-pid action, not by closing its window.

**Unified cross-tab key table.** All three tabs share one verb vocabulary so muscle memory transfers: `r` = refresh (every tab — rendered ONCE in the App-level `FOOTER_PREFIX` via `App.set_hints`, not per-view), `s` = 停止 (stop/kill a live process — Sessions / Agents / RC single-stop), `Enter` = the tab's primary action (Sessions / Agents 接回, also `o` / RC 启动), `t` = "bring the terminal into a tmux claude window" — tmux 接回 on Sessions (foreground tmux resume/attach, see above) and tmux 新建 on Projects (`do_tmux_new`: NEW `claude` in the project's own per-project tmux session, no `--remote-control`, nothing killed → no confirm/R10/trust gate, then `enter_window`), `R` = 转入后台 (relaunch-into-tmux) on Sessions and 重启 (respawn) on Agents, `d` = delete a settled record. **Confirm-on-kill is unified (not just two keys):** every op that terminates a live process confirms via the App-level `App.confirm(message, on_yes)` modal (routed in `_input`; `_confirm_yes` gates it) — Sessions `s` / `Enter`-live / `R`-live, Agents `s` / `Enter`-live, RC `s` (single, only when running) / `S` (all, skipped when none running). Whether `Enter`/`R` is a takeover (kills the old pid first) reads `session_ops.would_take_over` — the single `should_kill`, so the confirm gate never re-derives it; resuming/relaunching a DEAD session kills nothing and is NOT confirmed. The whole degrade-gate → confirm → act sequence lives ONCE in `views/_confirm.py` — `confirm_takeover` for takeovers and its plain-stop twin `confirm_stop` (degrade-gate → alive → current → confirm, 文案 derived from one noun; RC passes `gated=False` because its stop kills a tmux window, not a pid) — plus the `_DEGRADED` refusal string and the `stop_message` 文案. Views call these instead of re-inlining the steps. Non-kill ops stay single-key: Sessions/Agents `d` (delete settled), Agents `R` (respawn), `f` (fork). `f` 分叉 has a current-session guard (forking the current session would `execvp`-replace csctl, like `Enter`/`s`). Same key → same 汉字 across tabs (`s`=停止, `Enter`=接回); `R` (转入后台 vs 重启) and `t` (tmux 接回 vs tmux 新建) are deliberately two-meaning and registered so. The R10 degrade gate fires BEFORE the confirm and is takeover-aware: it refuses a live takeover off `/proc` but still allows relaunching a dead session. confirm 文案 follow one template — `{动词}{对象}「name」？{后果}` (接管类 "将先终止原进程。" / 停止类 "将终止其进程。"). `c` stays tab-specific (Sessions cleanup / RC toggle 自动远控) — not part of the universal verb set. RC `ServerRow`/`EnvRow` are `selectable() == False` (display-only; focus skips them).
