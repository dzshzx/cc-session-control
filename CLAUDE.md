...
- **`data/`**——所有触碰外部状态的代码，读*和*写都在这里。它内部是一个 **bottom→top DAG**（无环）：
  ...
  - top（组装）：`sessions.py`（把 typed transcript records + liveness/residency 投影为 `Session`，不拥有 transcript 文件 IO/解析）、`rc.py`（RC 领域逻辑：trust + `/proc` 服务器发现 → `RCProject`/`RCServer`；实际 tmux 调用消费 `tmux.py`）、`providers/`（**多 CLI provider 层，ADR-0005**：`base.py` typed capabilities/protocol、`claude.py`/`codex.py`/`kimi.py` 各一个 adapter、`argv_live.py` 共享的 argv-exact liveness 纯 join、`__init__.py` 注册表 + `scan_non_claude`（每 generation 一次 `proc.scan_cli_argv_inventory` `/proc` argv walk 服务全部非 Claude provider）+ `resolve_argv_execution`（非 Claude live takeover 的执行时重解析）。argv 合成（resume/fork/new/window 名）一律走 `providers.get(key)`，绝不在别处内联 CLI 命令；非 Claude 进程只有 argv 携带可证明的 session 目标才绑定——`codex resume <sid>`、经 `session_index.jsonl` thread_name→id 唯一反查绑定的 `codex resume <name>`（重名或未知名字不绑）、或 `kimi --session <sid>`；`exec` 前置于 `resume` 之前的 codex argv（`codex exec` 把 "resume" 读成 prompt 文本）绝不绑定；裸 TUI 与 daemons（app-server 等）绝不是 kill 目标；provider 来源故障降级为 `WorldSnapshot.provider_issues`，绝不清空 Claude 视图）。
