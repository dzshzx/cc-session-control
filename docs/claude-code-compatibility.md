...
| Isolated read-only command probe | 2.1.233 | 2026-08-17 | For the v0.8.12 release candidate: `claude --version`, `claude agents --help`, and `claude agents --json` all exited 0 under an empty temporary home; help still advertised `--json`, whose output parsed as a JSON array (`[]`) |
