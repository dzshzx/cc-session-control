...
| Kimi hook contract for the runtime registry: `SessionStart` fires when a session materializes (TUI first prompt; immediately for `--prompt`) with payload `session_id`/`cwd`/`source` (startup\|resume); `SessionEnd` fires on clean `/exit` with `reason`; the hook process is kimi's grandchild (kimi → `sh -c` → hook) | 0.34.0 | 2026-08-12 | Temporary `[[hooks]]` probe writing ppid/payload to a log, driven through TUI (tmux), `-p`, and `--session` resume; config restored byte-identical afterwards |
