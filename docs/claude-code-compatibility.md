...
| Live `sessions/*.json` `procStart` presence | 2.1.241 | 2026-08-23 | Read-only field-presence scan of the operator's real `~/.claude/sessions/*.json` (`python3 -c`, counts only, no contents printed): 5/5 rows carried a non-empty string `procStart`; 0 missing/empty. No prior evidence row recorded this field's presence rate directly — this backs `registry._probe_proc_liveness`/`live_session_procs` treating a missing `procStart` as schema drift (typed issue + `proc_alive=None`), not a routine shape |
| `sessions/<pid>.json` `bridgeSessionId` lifecycle (session Remote Control) | not recorded | 2026-06-27 | Observed on the maintainer's machine while building the liveness model (commit d0ed8d0; moved here from `CONTEXT.md` on 2026-08-23, not re-run since): enabling Remote Control on a session mints a NEW environment id; disabling sets `bridgeSessionId` to `null` — a transient state seen on disconnect and overwritten by a fresh id on the next enable, so a random snapshot usually shows only absent-or-string; re-enabling mints ANOTHER new id. The file keeps only the current binding (single field, overwritten), so toggled-away environments leave no structured trace on disk (only noisy mentions in transcripts), and Claude Code exposes no local command to deregister a cloud/mobile entry — deletion stays manual on claude.ai/code. The other upstream namespaces seen then — `cse_*` in `jobs/*/state.json`, `env_*` on a project RC server's stdout — are no longer read by csctl (ADR-0009) |
...
| A running codex process carries its home in `CODEX_HOME` (absent = codex's `~/.codex` default); two identities are otherwise indistinguishable, since a launcher that `exec`s codex leaves argv0 as plain `codex` | 0.147.0 | 2026-08-13 | `/proc/<pid>/environ` of live codex processes (app-server pids `<pid>`/`<pid>` → `<operator-home>/.codex`; one Windows-side process → its own home) |
