...
Full development guide (architecture, commands, invariants) lives in `CLAUDE.md` — reading @CLAUDE.md once at the start of a session is recommended; no need to re-read it later in the same session. `CONTEXT.md` is the domain glossary (Live Session, Bridge Environment, …).
