...
class SessionsView(CleanupMixin, ListTabView):
    ...
    HELP_LAYOUT = HelpLayout(
        prefix=(
            ...
            "        ● 活 = 运行中（cx/km/oc 专属：已绑定进程，忙闲不可知）·",
            "        ○ 停 = 无进程（cx/km/oc 行为「未发现可绑定进程」）·",
            "        @ 托管 = Codex app-server 持有（只读，不接回/停止/删除）·",
            "        ? 未知 = 疑似运行（cx/km/oc 专属：目录内有未绑定进程，",
            "        可能正持有该会话，接回/转后台会先确认双开风险）",
            "        ▸ = 当前会话（启动 csctl 的会话，受保护） · 📱 = 已开远控",
            "        ⧉ = tmux 驻留（会话进程在 tmux 窗口里，断线不死）",
            "        ? = tmux 驻留未知（盘点不完整，不能确认驻留或裸终端）",
            "CLI 列: cc = Claude Code · cx = Codex · km = Kimi Code · oc = opencode",
            "        （providers.json 声明的多个 codex 身份各用自己的标签，",
            "        如 cx2，会话/信任/进程互不混淆，见 ADR-0008）",
            "        （codex 绑定 resume argv 或可信派发元数据；kimi 另可经",
            "        hook 注册表绑定已登记会话（含裸启动），新建会话首个",
            "        prompt 前无 sid；opencode 绑定 --session argv 或可信派发",
            "        元数据（无 hook 注册表）；无证据进程不会被停止/接管，",
            "        见 ADR-0005）",
            "",
        ),
        sections=("会话操作:", "清理与过滤:"),
